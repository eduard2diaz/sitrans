var provincia = function () {
    ...
    var configurarDataTable = function () {
        table = $("table#provincia_table").DataTable(
            {
                responsive:true,
                //   searchDelay:500,
                //  processing:true,
                //    serverSide:true,
                ajax: Routing.generate('provincia_index'),
                "language": {
    ...
    var configurarFormulario=function(){
        $("div#basicmodal form").validate({
            rules:{
                'provincia[nombre]': {required:true}
            },
    ...
    var newAction = function () {
        $('div#basicmodal').on('submit', 'form#provincia_new', function (evento)
        {
            evento.preventDefault();
    ...
    var edicionAction = function () {
        $('div#basicmodal').on('submit', 'form#provincia_edit', function (evento)
        {
            evento.preventDefault();
            var padre = $(this).parent();
            var l = Ladda.create(document.querySelector( '.ladda-button' ) );
    ...
    var eliminar = function () {
        $('table#provincia_table').on('click', 'a.eliminar_provincia', function (evento)
        {
           ...
           bootbox.confirm({
                ...
                message: "<p>¿Está seguro que desea eliminar esta provincia?</p>",
                buttons: {
                    confirm: {
                ...
                callback: function (result) {
                    if (result == true)
                        $.ajax({
                            type: 'get', //Se uso get pues segun los desarrolladores de yahoo es una mejoria en el rendimineto de las peticiones ajax
                            // dataType: 'html', esta url se comentprovincia porque lo k estamos mandando es un json y no un html plano
                            url: link,
                            beforeSend: function () {
                                mApp.block("body",
